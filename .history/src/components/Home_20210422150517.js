...
const Home = () => {
    ...
    let wdeaths = 0;
    function calculate_total_results(){
        Covid_Data && Covid_Data.map((c) => {
            wdeaths += c.latest_data.deaths;
        });
        setWorldTotalDeaths(wdeaths);
    }

    const [WorldTotalDeaths, setWorldTotalDeaths] = calculate_total_results(0);
