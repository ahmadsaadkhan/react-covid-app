            ...
            {Covid_Data && Covid_Data.map((c) => (
                ...
                c.name === country ? (
                    <div className="section" key={c.code}>
                        ...
                        <p className="mt-2 mb-2">Cases 1M population {c.latest_data.calculated.cases_per_million_population}</p>
                        <p className="mt-2 mb-2">Death Rate {c.latest_data.calculated.death_rate}</p>
                        <p className="mt-2 mb-2">Recovery Rate {c.latest_data.calculated.recovery_rate}</p>
...
const Total = ({ Covid_Data }) => (
    <h3>
      Price: 
      {Covid_Data.reduce((sum, i) => (
        sum +=  i.latest_data.deaths
      ), 0)}
    </h3>
  )
